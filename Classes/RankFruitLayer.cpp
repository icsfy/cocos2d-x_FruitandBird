...
bool RankFruitLayer::init() {  // 初始化
  if (!Layer::init())
    return false;
  auto visibleSize = Director::getInstance()->getVisibleSize();
  Vec2 origin = Director::getInstance()->getVisibleOrigin();

  Sprite* backGround = Sprite::create("pic/morning.png");  // 背景
  ...
  Sprite* title = Sprite::create("pic/rankFruit.png");  // 创建标题
  title->setPosition(Vec2(270, 850));  // 设置精灵位置
  this->addChild(title, 1);  // 添加到布景中

  auto menuItem = MenuItemImage::create(  // 切换到主菜单
    "button/menu.png", "button/menu_off.png", CC_CALLBACK_1(RankFruitLayer::menuCallBack0, this));
  menuItem->setPosition(Vec2(80, 50));
  auto nextItem = MenuItemImage::create(  // 切换到下一个排行榜
    "button/back.png", "button/back_off.png", CC_CALLBACK_1(RankFruitLayer::menuCallBack1, this));
...
void RankFruitLayer::load() {  // 读取分数
  for (int i = 0; i < 5; i++) {
    scoreFruit[i] = UserDefault::getInstance()->
            getStringForKey(StringUtils::format("f%d", i).c_str(), "0");
  }
}

void RankFruitLayer::menuCallBack0(Ref* pSender) {  // 切换到主菜单场景的菜单项回调方法
  sceneManager->goToMainScene();  // 切换到主菜单场景
}

void RankFruitLayer::menuCallBack1(Ref* pSender) {  // 切换到下一个排行榜场景的菜单项回调方法
  sceneManager->goToRankBirdScene();  // 切换到"进击的小鸟"排行榜场景
}
